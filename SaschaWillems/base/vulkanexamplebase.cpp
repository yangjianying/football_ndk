...
void VulkanExampleBase::renderFrame__android() {
#if defined(VK_USE_PLATFORM_ANDROID_KHR)
...
void VulkanExampleBase::renderLoop()
{
	...
	while (1)
	{
		int ident;
		int events;
		struct android_poll_source_ * source;
		bool destroy = false;
		...
		if (destroy)
		{
			// ANativeActivity_finish(androidApp->activity);  // frankie, add
			break;
		}
...
#elif defined(VK_USE_PLATFORM_ANDROID_KHR)
int32_t VulkanExampleBase::handleAppInput(struct android_app_* app, AInputEvent* event)
{
...
void VulkanExampleBase::handleAppCommand(android_app_ * app, int32_t cmd)
{
