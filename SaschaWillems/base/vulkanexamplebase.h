...
#elif defined(VK_USE_PLATFORM_ANDROID_KHR)
	static int32_t handleAppInput(struct android_app_* app, AInputEvent* event);
	static void handleAppCommand(android_app_* app, int32_t cmd);
#elif (defined(VK_USE_PLATFORM_IOS_MVK) || defined(VK_USE_PLATFORM_MACOS_MVK))
...
{ \
 ...
 \
	...
	state->onAppCmd = name::VulkanExample::handleAppCommand; \
	state->onInputEvent = name::VulkanExample::handleAppInput; \
	androidApp = state; \
	vulkanExample->renderLoop(); \
	delete(vulkanExample); \
}
