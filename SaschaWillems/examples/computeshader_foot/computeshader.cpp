...
// here, will result the low fps , as the compute shader and graphics shader is executed serially!
...
#if 0  // frankie, for 1080 x 1920 
			vkCmdDispatch(drawCmdBuffers[i], textureComputeTarget.width / 18, textureComputeTarget.height / 31, 1);
#else
	...
	#if 0  // frankie, for 1080 x 1920 
		vkCmdDispatch(compute.commandBuffer, textureComputeTarget.width / 18, textureComputeTarget.height / 31, 1);
	#else
...
#if 0  // frankie, modify the compute shader !
		shaderNames = { "emboss_bak", "edgedetect", "sharpen" };
#else
...
VULKAN_EXAMPLE_MAIN(computeshader_foot, "/sdcard/data/SaschaWillems/computeshader_foot/assets/")
