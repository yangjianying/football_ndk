...
void VulkanExample::InitExtra() {
	if (mInitFlag == INIT_HISTOGRAM_GRAPHIC) {
		mHistogramGraphics = new HistogramGraphics(this);
	}
	else {
		mHistogramCompute = new HistogramCompute(this);  // frankie, add
	}

	if (mInitFlag >= INIT_COMPUTE_START && mInitFlag < INIT_COMPUTE_END) {
		// normal screen , close the UI
...
void VulkanExample::setDebugWindow(int enable) {
	if (mHistogramGraphics != nullptr) { mHistogramGraphics->setDebugWindow(enable); }
	if (mHistogramCompute != nullptr) { mHistogramCompute->setDebugWindow(enable); }
}
void VulkanExample::impl1_setBHE_factor(float f0, float f1) {
	if (mHistogramCompute != nullptr) { mHistogramCompute->setBHE_factor(f0, f1); }
}
...
VulkanExample::~VulkanExample()
{
	...
	if (mHistogramCompute != nullptr) {  // frankie, add
		delete mHistogramCompute;
		mHistogramCompute = nullptr;
	}
	if (mHistogramGraphics != nullptr) {  // frankie, add
		delete mHistogramGraphics;
		mHistogramGraphics = nullptr;
	}
...
#endif
		if (mHistogramCompute != nullptr) { mHistogramCompute->appendCommandBuffers(drawCmdBuffers[i]); }
		if (mHistogramGraphics != nullptr) { mHistogramGraphics->appendCommandBuffers(drawCmdBuffers[i]); }
...
#if USE_HistogramCompute_target
		if (mHistogramCompute != nullptr) {
			vks::Texture2D & target_ = mHistogramCompute->getTextureTarget();
...
void VulkanExample::prepare()
{
	...
	prepareCompute();
	if (mHistogramCompute != nullptr) {
		mHistogramCompute->mPhysicalDevice = physicalDevice;
		mHistogramCompute->mDevice = device;
		mHistogramCompute->mPipelineCache = pipelineCache;
		mHistogramCompute->prepare();
...
void VulkanExample::draw()
{
	...
	if (mHistogramCompute != nullptr) { mHistogramCompute->draw(); }
	if (mHistogramGraphics != nullptr) { mHistogramGraphics->draw(); }
...

{
	...
	if (mHistogramCompute != nullptr) {
		mHistogramCompute->updateSource_VkDescriptorImageInfo(&importTexture, 
			hardwareBufferDesc.width, hardwareBufferDesc.height, &texDst);
	}
	if (mHistogramGraphics != nullptr) {
		mHistogramGraphics->updateSource_VkDescriptorImageInfo(&importTexture, 
			hardwareBufferDesc.width, hardwareBufferDesc.height, &texDst);
	}
}
