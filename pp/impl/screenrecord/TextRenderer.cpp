...
float TextRenderer::computeScaledStringWidth(/*const String8& str8*/char *str) {
...
float TextRenderer::computeScaledStringWidth(char* str,
        size_t len) {
    float width = 0.0f;
...
void TextRenderer::drawString(const Program& program, const float* texMatrix,
        float x, float y, /*const String8& str8*/ char *str) const {
...
float TextRenderer::drawWrappedString(const Program& texRender,
        float xpos, float ypos, /*const String8& str*/ char *str) {
    ALOGV("drawWrappedString %.3f,%.3f '%s'", xpos, ypos, /*str.string()*/str);
    initOnce();
    ...
    if (stringWidth <= maxWidth) {
        ...
        // boundaries.
        char* mangle = strdup(/*str.string()*/str);
        char* start = mangle;
        while (start != NULL) {
...
char* TextRenderer::breakString(char* str, float maxWidth) {
