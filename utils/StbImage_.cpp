...
StbImage_::StbImage_(char const *filename, int tWidth, int tHeight) {
	...
	DLOGD( "StbImageLoader size: %04d x %04d n = %d \r\n", iw, ih, n);
	w = iw;
	h = ih;
	...
	if(tw > 0 && th > 0 && (tw != w || th != h)) {
		DLOGD( "  target size is %4d x %4d , resize it! \r\n", tw, th);
		int ow = tw;
		int oh = th;
		...
		DLOGD( "  target size is the same as original size , no need to resize ! \r\n");
	}
	if (tw < 0) {
		tw = w;
	}
...
/*static*/int StbImage_::test1() {
	...
	DLOGD( "idata size: %04d x %04d n = %d \r\n", iw, ih, n);
