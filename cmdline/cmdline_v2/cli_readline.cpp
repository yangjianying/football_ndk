...
const char *hist_::hist_prev(void)
{
	char *ret;
	int old_cur;
...
const char *hist_::hist_next(void)
{
	char *ret;
...
#if 0  //#ifndef CONFIG_CMDLINE_EDITING
...
void cli::cread_add_char(char ichar, int insert, unsigned long *num,
...
void cli::cread_add_str(char *str, int strsize, int insert,
...
int cli::cread_line(const char *const prompt, char *buf, unsigned int *len,
		int timeout)
{
	unsigned long num = 0;
	...
	while (1) {
		...
		case CTL_CH('n'):
		{
			...
			if (ichar == CTL_CH('p'))
				hline = hist->hist_prev();
			else
				hline = hist->hist_next();
	...
	*len = eol_num;
	...
	if (buf[0] && buf[0] != CREAD_HIST_CHAR)
		hist->cread_add_to_hist(buf);
	hist->hist_cur = hist->hist_add_idx;
...
int cli::cli_readline(const char *const prompt)
{
	/*
...
int cli::cli_readline_into_buffer(const char *const prompt, char *buffer,
			     int timeout)
{
	char *p = buffer;
#if 1  // #ifdef CONFIG_CMDLINE_EDITING
